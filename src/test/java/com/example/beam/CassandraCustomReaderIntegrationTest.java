...
import com.example.beam.CassandraCustomReader.CassandraRead;
...
/**
 * Integration tests for CassandraCustomReader against a real Cassandra instance.
 *
 * <p>Requires Cassandra 4.1 reachable at localhost:9042. Start it with:
 *
 * <pre>
 *   docker-compose up -d cassandra
 * </pre>
 *
 * Cassandra takes ~45-60 s to become ready. The @BeforeClass setup retries for up to 60 s.
 */
@RunWith(JUnit4.class)
public class CassandraCustomReaderIntegrationTest {
  ...
  /**
   * Builds and runs a pipeline using CassandraRead with the given bounds, writes CSV output to
   * {@code outputDir}, and returns the saga_id values from all output lines.
   */
  ...
  @Test
  public void testBothBoundsIncludeBoundaryRows() throws Exception {
    List<String> sagaIds =
    ...
    Assert.assertTrue("saga1 (dlqTs=1500) should be included", sagaIds.contains("saga1"));
    Assert.assertTrue("saga2 (dlqTs=1000, lower bound) should be included", sagaIds.contains("saga2"));
    Assert.assertTrue("saga3 (dlqTs=2000, upper bound) should be included", sagaIds.contains("saga3"));
    Assert.assertFalse("saga4 (dlqTs=999) should be excluded", sagaIds.contains("saga4"));
    Assert.assertFalse("saga5 (dlqTs=2001) should be excluded", sagaIds.contains("saga5"));
  }
  ...
  public void testLowerBoundOnlyExcludesRowsBelow() throws Exception {
    ...
    Assert.assertTrue(sagaIds.contains("saga2")); // 1000 >= 1000 (inclusive)
  ...
  public void testUpperBoundOnlyExcludesRowsAbove() throws Exception {
    ...
    Assert.assertTrue(sagaIds.contains("saga3")); // 2000 <= 2000 (inclusive)
  ...
  @Test
  public void testExactPointRangeMatchesSingleRow() throws Exception {
    // saga2 has dlqTs=1000; a range of [1000, 1000] should match only it
    List<String> sagaIds =
