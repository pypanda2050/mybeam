...
/**
 * Integration test that runs the pipeline against local Docker services.
 * Requires:
 * - Postgres at localhost:5432 (user: test, pass: test, db: testdb)
...
public class PostgresToGcsPipelineIntegrationTest {
    ...
    public void setup() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(POSTGRES_URL, props);
            ...
            stmt.execute("CREATE TABLE dlq_job (" +
                    "saga_id VARCHAR(255), " +
                    "node_id VARCHAR(255), " +
                    "create_ts BIGINT, " +
                    "dlq_ts BIGINT)");
            ...
            stmt.execute(String.format("INSERT INTO dlq_job VALUES ('saga-int', 'node-int', %d, %d)", now, recentTs));
        }
